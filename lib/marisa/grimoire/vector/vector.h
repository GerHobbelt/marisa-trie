...
class Vector {
 ...
 private:
  ...
  static T *castPointer(std::byte *ptr) {
    // In C++17, `std::launder` appears to be needed to avoid UB according to
    // Example 2 in https://en.cppreference.com/w/cpp/utility/launder,
    // where similar code without launder is flagged as UB.
    //
    // In C++20, `std::launder` is not needed, but does not hurt:
    // https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2020/p0593r6.html#objects-provided-as-byte-representation
